...
# 		energy_noise = np.mean((sig[og - wlen_pre:og]) ** 2)
...
def snr_from_trace(tr, origin_times, win_lims):
	sr = tr.stats.sampling_rate
	origins = (origin_times * sr).astype(int)
	wlen_pre, wlen_post = (win_lims * sr).astype(int)
	snrs = compute_snr(tr.data, origins, wlen_pre, wlen_post)
	return snrs
